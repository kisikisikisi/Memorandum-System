...
(chunk-type (action (:include operator)) action post)
(chunk-type (detailed-action (:include action)) required label)
(chunk-type (test (:include operator)) slot success failure)
(chunk-type task state step context)
(chunk-type result)

(add-dm
  (op1 isa detailed-action pre start         action read                           label   word    post stimulus-read)
  (op2 isa detailed-action pre stimulus-read action retrieve      required word    label   number  post recalled)
  (op3 isa test            pre recalled      slot   number        success  respond failure wait)
  (op4 isa detailed-action pre respond       action type          required number                  post wait)
  (op5 isa detailed-action pre wait          action read                           label   number  post new-trial)
