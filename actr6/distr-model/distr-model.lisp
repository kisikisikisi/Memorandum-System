...
(defvar *directory* "/actr6/distr-model/")
(defvar *extension* "/create-chunk/history/")
(defvar *time*)
  ...
  (let ((chunks))
    (dotimes (i coun)
      (with-meta-process p1
		 ...
		 (load (format nil "~Aparam/looking.lisp" *directory*)))
...
(defun run-pic (num)
  ...
  (with-meta-process p1
	...
	;(format t "g1-0" (goal-focus));ゴールバッファの中身確認
...
(defparameter *model-code* 
	'((sgp :esc t ;enable simbolic computation
		...
		;:act t ;debug
	    ;:v "C:/Users/bi17030/Documents/Research/actr6/actr6/distr-model/confirm.txt"
	        :v nil
    ...
    (chunk-type inc-label idata label looking1 looking2 looking3)
    ;;(chunk-type photo-mood idata mood)
    (chunk-type label-data text)
    (chunk-type image-data text type)
	;; 3.1.2 Goal definition
    (chunk-type assoc current nimage label time state
	 phase looking1 looking2 looking3)
...
;;3.2 Rule parameters
(spp rt-from-label :u 10 :at 0.05)
